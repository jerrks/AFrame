...
public class AsyncLoader<K, T, V, E, R> {
	...
	public AsyncLoader(int capacity, int maxWorker,
			LoaderProxy<K, T, V, E, R> proxy) {
		if (maxWorker < 1)
	...
	class Worker extends Thread {
		...
		public void run() {
			while (true) {
				...
				if (node != null) {
					if(mWorkTag == mTag){
						R data = mProxy.doInBackground(node.key, node.param,
								node.obj, node.extra);
						node.data = data;
